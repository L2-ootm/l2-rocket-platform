...
3. `SOURCE_MAP.md` — which files are the active pipeline, which are retired/legacy, and known repo-hygiene debt (large committed files, untracked entry-point scripts, stale duplicate docs). Do not trust `OSIFOG/SOURCE_MAP.md` or treat `osifog_engine_search.py`/`osifog_podset.py`/`osifog_sweep.py`/`osifog_precision.py` as current — they are retired predecessor pipelines. The active pipeline is `rocket_ast.py` → `l2_engine` (Rust) → `organic_loop.py` → `organic_campaign.py` → `osifog_campaign_watchdog.py`.
...
- Secondary, lower priority: this repository also has known cleanup/repo-hygiene debt (see `SOURCE_MAP.md`) — a 1.1GB file committed to git history, untracked entry-point scripts, ~7.3GB of retired campaign output, stale duplicate documentation. Only spend budget on this if the primary goal (search quality) is in good shape and time remains — do not let repo cleanup crowd out the actual optimization work.
